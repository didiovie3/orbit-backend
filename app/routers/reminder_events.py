...
def _get_owned_reminder_event(supabase: Client, event_id: UUID, user_id: str) -> dict:
    ...
    """
    result = (
        supabase.table("reminder_events").select("*").eq("id", str(event_id)).maybe_single().execute()
    )
    if not result.data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Reminder event not found")

    task_result = (
        supabase.table("tasks").select("user_id").eq("id", result.data["task_id"]).maybe_single().execute()
    )
...
def update_reminder_event(
    ...
    result = supabase.table("reminder_events").select("*").eq("id", str(event_id)).maybe_single().execute()
    return result.data
